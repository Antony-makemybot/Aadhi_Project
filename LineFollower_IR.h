...
#define RELAY_PIN 7
#define IR_SENSOR_PIN 6  // IR sensor for detecting obstacles
...
unsigned long lastIRSignalTime = 0; // To time out after the second signal
...
void manageIRSensor() {
    ...
    if (irSignalValue == HIGH) { // If the IR sensor detects an obstacle
        if (!firstSignalReceived) {
            ...
            lastIRSignalTime = currentMillis; // Record the time of the first signal
        } else {
            // Check if a second signal has been received within a reasonable time
            if (currentMillis - lastIRSignalTime >= 2000) { // 2 seconds for a second signal
                myServo.write(0); // Tuck servo back to 0 degrees
                firstSignalReceived = false; // Reset for the next cycle
            }
        }
    } else {
        // If IR signal is low and we're within the timeout, reset the first signal
        if (firstSignalReceived && (currentMillis - lastIRSignalTime >= 2000)) {
